...
class ProxyRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        is_js =  res.headers.get('Content-Type', '').startswith('text/javascript') or res.headers.get('Content-Type', '').startswith('application/javascript') #self.detect(res, res_body_plain)
        if is_js:
            is_malicious = self.detect.predict(res_body)
            print is_malicious
            if is_malicious:
                with open(str(random.randint(1000,9999))+'.txt', 'w') as f:
                    f.write(res_body)
                res_body = "alert('WARNING!!! This website contains malicious javascript code');"
                res.headers['Content-Length'] = str(len(res_body))
        ...
        if res_body is not None:
            ...
            if content_type.startswith('application/json'):
                ...
                with open(filename+'.txt', 'w') as f:
                    f.write(res_body)
                #res_body_text = res_body
                #print with_color(32, "==== JAVASCRIPT FILE ====\n%s\n" % res_body)
            elif content_type.startswith('text/') and len(res_body) < 1024:
                res_body_text = res_body

            if res_body_text is not None:
...
def start(HandlerClass=ProxyRequestHandler, ServerClass=ThreadingHTTPServer, protocol="HTTP/1.1"):
    if sys.argv[1:]:
        port = int(sys.argv[1])
    else:
        port = 8080
